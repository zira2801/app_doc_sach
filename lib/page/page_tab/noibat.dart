...
class _NoiBatWidgetState extends State<NoiBatWidget> {
  ...
  List<Book> _booksNoiBat = [];

  Future<void> _loadBooks() async {
    try {
      final books = await _bookService.getBooksByStatus('Nổi bật');
  ...
  Widget gridview_noibat() {
    ...
    if (_bookColors.length != _booksNoiBat.length) {
      _bookColors = List<Color>.generate(_booksNoiBat.length, (index) => getRandomColor());
    }
    return GridView.builder(
      ...
      itemCount: _booksNoiBat.length,
      itemBuilder: (context, index) {
        final book = _booksNoiBat[index];
