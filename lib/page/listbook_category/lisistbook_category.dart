...
class ListBookCategory extends StatefulWidget {
  ...
  const ListBookCategory({super.key,required this.category});
...
class _ListBookCategoryState extends State<ListBookCategory> {
  ...
  Future<void> _loadBooksByCategory() async {
    ...
    try {
      List<Book> books = await _bookService.getBooksByCategory(widget.category.nameCategory);
      ...
      setState(() {
        _booksbyCategory = books;
        _isLoading = false; // Set loading state false
        if (_booksbyCategory.isEmpty) {
  ...
  String getFirstCategoryName(List<CategoryModel> categories, CategoryModel category) {
    if (categories.isNotEmpty) {
      // Tìm category chính xác trong danh sách
      CategoryModel? matchedCategory;
      try {
        matchedCategory = categories.firstWhere((cat) => cat.id == category.id);
      } catch (e) {
        // Xử lý lỗi nếu không tìm thấy category phù hợp
        print('Không tìm thấy category phù hợp: $e');
      }

      // Nếu tìm thấy thì trả về tên của category
      if (matchedCategory != null) {
        return matchedCategory.nameCategory;
      }
    }

    // Nếu không tìm thấy hoặc danh sách rỗng, trả về chuỗi rỗng
    return '';
  }
      ...
      appBar: PreferredSize(
        ...
        child: Padding(
          ...
          child: AppBar(
            ...
            title: Text(
              'Thể loại: ${widget.category.nameCategory}',
  ...
  Widget gridview_bookcategory() {
    ...
    if (_bookColors.length != _booksbyCategory.length) {
      _bookColors = List<Color>.generate(_booksbyCategory.length, (index) => getRandomColor());
    }
    if (_isLoading) {
      ...
      return GridView.builder(
        ...
        itemBuilder: (context, index) {
          final book = _booksbyCategory[index];
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: Container(
                          child: Text(
                            getFirstCategoryName(book.categories ?? [], widget.category),
